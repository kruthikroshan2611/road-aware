...
import { Camera, MapPin, CheckCircle2, AlertTriangle, RotateCcw, ImageIcon, Loader2, Sparkles, Brain } from "lucide-react";
...
const ReportDamage = () => {
  ...
  const analyzeImageWithAI = async (imageUrl: string) => {
    ...
    try {
      ...
      if (data?.success && data?.analysis) {
        ...
        // Auto-fill the damage type and severity from AI suggestion
        setFormData((prev) => ({
          ...prev,
          damageType: data.analysis.damageType,
          severity: data.analysis.severity,
        }));

        toast({
          title: "AI Analysis Complete",
          description: `Detected: ${data.analysis.damageType} (${data.analysis.severity} severity) - ${Math.round(data.analysis.confidence * 100)}% confidence`,
        });
      }
    } catch (error: any) {
      ...
      toast({
        ...
        description: "Could not analyze image. Please select damage type manually.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleRetakeImage = () => {
    ...
    setFormData((prev) => ({ ...prev, damageType: "", severity: "" }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If AI hasn't analyzed yet, try to analyze now
      if (!aiSuggestion && !isAnalyzing) {
        await analyzeImageWithAI(imageUrl);
      }

      // Insert report - complaint_id will be auto-generated by database trigger
      ...
      // Verify complaint_id was generated
      if (!data?.complaint_id) {
        throw new Error("Complaint ID was not generated. Please try again.");
      }
                      ...
                      <>
                        ...
                        {isAnalyzing && (
                          <div className="flex items-center justify-center gap-2 p-3 bg-primary/10 rounded-lg">
                            ...
                            <span className="text-sm text-primary">Analyzing damage...</span>
                          </div>
                        )}
